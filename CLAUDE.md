...
设置面板(`SettingsPanel.vue`)把这两件事放在一起,因为它们互相牵制:改切片参数会让
已切过的图按新参数重切一份,而清缓存是抹掉所有旧瓦片 —— 用户得在同一个地方看到这个因果。

三条不能破的:

- **缓存占用必须给两个数**(全局 `cache_stats` ↔ 当前图的 `image.cacheBytes`)。
  只给一个的话,人会拿当前图的大小去估计能清出多少空间。状态栏那一格是**当前这张**,
  它是清理入口,但不是清理的量。
- **清缓存必须把当前图一起关掉**(`onCacheCleared` → `viewerApi.close()`)。瓦片文件
  已经被删了,留着图只会是一片空白,而且用户不知道发生了什么。标注不受影响 ——
  它存在图片旁边,是另一个地方。切片进行中还要禁用清理按钮:正在写入的目录会被删到一半。
- **切片参数进缓存键**(`cache_key`),所以改参数 = 重切,而**旧的那份不会自动删**。
  界面必须把这句话说出来,否则用户看到的只是「改了个设置,下次打开图又等了几分钟」。

前端**总是显式**把参数发过去(`state/slicePrefs.ts`),默认值刻意和 Rust 侧的
`DEFAULT_TILE_SIZE` 对齐 —— 两边不一致的话,「没设置过」和「显式设成默认」会命中两份缓存。
