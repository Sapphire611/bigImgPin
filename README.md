...
工具栏最右边的齿轮(或者点状态栏「瓦片缓存」那一格)打开设置:

- **缓存占用**给两个数:**全部图片**加起来,和**当前图片**这一张。
  只给一个的话,人会拿当前图的大小去估计能清出多少空间 —— 那多半差着几十倍。
- **清理**。正开着图时按钮写的是「清理全部缓存并关闭当前图片」,它会**连图一起关掉**:
  瓦片文件都删了,留着只会是一片空白。标注不受影响,它存在图片旁边,是另一个地方。
- **切片参数**:瓦片尺寸(512 / 1024 / 2048)和格式(WebP 无损 / PNG)。
  只影响**之后打开**的图;改完再打开已经切过的图,会按新参数重切一份,
  而**旧的那份不会自动删**(它躺在磁盘上,要清就上面那个按钮)。

没给 JPEG 选项:边缘锐利的图上有振铃,可能被误读成真实结构 —— 见上文「为什么默认 WebP 无损」。
...
整个金字塔共 **2746 张瓦片**(实测数过)。

> `image_files/` 里还有个 `vips-properties.xml`,是 vips 写的元数据,**不是瓦片** ——
> 直接数目录里的文件会多出这一个。

前端只按当前视野和缩放级别请求需要的那几十张,所以**图有多大都不影响交互** ——
一次视野变化只跟瓦片尺寸和磁盘速度有关。
